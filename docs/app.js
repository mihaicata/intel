...
const REPO = "mihaicata/intel";
$("#workflowLink").href = `https://github.com/${REPO}/actions/workflows/refresh-data.yml`;
...
function renderAdvisories(el, items, err) {
  if (err) { el.innerHTML = `<div class="err">Snapshot unavailable: ${esc(err)}</div>`; return; }
...
// ---- Lookup tool (live — Shodan InternetDB + ipwho.is both allow CORS) ----
...
async function doLookup() {
  ...
  try {
    const [idb, geo] = await Promise.allSettled([
      getJson(`https://internetdb.shodan.io/${ip}`),
      getJson(`https://ipwho.is/${ip}`),
    ]);
    const idbData = idb.status === "fulfilled" ? idb.value : null;
    const geoData = geo.status === "fulfilled" && geo.value.success !== false ? geo.value : null;

    const geoLine = geoData ? `${geoData.country} (${geoData.country_code}) · ${geoData.city || "?"}` : "unknown";
    const ports = idbData?.ports?.length ? idbData.ports.join(", ") : "none observed";
    const vulns = idbData?.vulns?.length ? idbData.vulns.join(", ") : "none known";
    const hostnames = idbData?.hostnames?.length ? idbData.hostnames.join(", ") : "none";
    out.innerHTML = `<strong>${esc(ip)}</strong>  ·  ${esc(geoLine)}\nOpen ports: ${esc(ports)}\nKnown vulns: ${esc(vulns)}\nHostnames: ${esc(hostnames)}`;
  } catch (e) {
...
async function loadAll() {
  ...
  const results = await Promise.allSettled([
    getJson("data/advisories.json"),
    getJson("data/kev.json"),
    getJson("data/cves.json"),
    getJson("data/iocs.json"),
    getJson("data/meta.json"),
  ]);

  const [adv, kev, cves, iocs, meta] = results;

  const advData = adv.status === "fulfilled" ? adv.value : null;
  renderAdvisories($("#advUs"), advData?.us?.items, adv.status === "rejected" ? adv.reason.message : advData?.us?.error);
  renderAdvisories($("#advRo"), advData?.ro?.items, adv.status === "rejected" ? adv.reason.message : advData?.ro?.error);

  renderKev($("#kevTable"), kev.value?.items, kev.status === "rejected" ? kev.reason.message : kev.value?.error);
  renderCves($("#cveTable"), cves.value?.items, cves.status === "rejected" ? cves.reason.message : cves.value?.error);

  if (iocs.status === "fulfilled" && !iocs.value.error) {
    iocCache = iocs.value.items || [];
    renderIocs();
  } else {
    $("#iocTable").innerHTML = `<div class="err">Snapshot unavailable: ${esc(iocs.status === "rejected" ? iocs.reason.message : iocs.value.error)}</div>`;
  }
  ...
  if (meta.status === "fulfilled" && meta.value.generatedAt) {
    const d = new Date(meta.value.generatedAt);
    $("#lastUpdated").textContent = `Data snapshot: ${d.toLocaleString()} (${timeAgo(meta.value.generatedAt)})`;
  } else {
    $("#lastUpdated").textContent = "Snapshot time unavailable";
  }
